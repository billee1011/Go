...
type PlayerModel struct {
	BaseModel
	players []*room2.Player
}
func (model PlayerModel) GetName() string{
	return models.Player
}
func (model PlayerModel) Start(){
	model.players = make([]*room2.Player,model.GetDesk().GetConfig().Num)
}
func (model PlayerModel) Stop(){

}

func (model PlayerModel) PlayerEnter(player *room2.Player,seat uint32){
	player.SetSeat(seat)
	player.EnterDesk(model.GetDesk())
}

func (model PlayerModel) PlayerQuit(player room2.Player){
	player.QuitDesk(model.GetDesk())
}

func (model PlayerModel) GetDeskPlayers() []*room2.Player {
