...
// Run 启动配牌
func RunPeiPai(addr string) error {
	// PP_PORT = addr
	http.HandleFunc("/", Peipai)
	http.HandleFunc("/option/", Option)
	http.HandleFunc("/setgold/", SetGoldHandle)
...
func SetGoldHandle(resp http.ResponseWriter, req *http.Request) {
	...
	coin, err = goldclient.AddGold(playerID, int16(server_gold.GoldType_GOLD_COIN), int64(int64(gold)-coin), 0, 0)
	if err != nil {
