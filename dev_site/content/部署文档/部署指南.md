...
rpc_addr | string | 提供的 rpc 服务 IP | 空 | 127.0.0.1 |  有部分服务不提供 rpc 服务， 不需要配置此项
...
consul_addr | string | consul agent 的地址 | 127.0.0.1:8500 | 127.0.0.1:8500 | 
nsq_addr | string | nsqd 监听的 tcp 地址 | 127.0.0.1:4150 | 127.0.0.1:4150 | 
nsqlookupd_addrs | list | nsqlookupd 的 HTTP 监听地址 | 127.0.0.1:4161 | 127.0.0.1:4161 127.0.0.1:4162 | 需要用 yaml 格式配置所有的 nsqlookupd http 地址
redis_addr | string | redis 服务地址 | 127.0.0.1:6379  | 127.0.0.1:6379 | 部分服务不需要配置此项。具体由[通用配置](/部署文档/部署指南/) 说明
...
* 不需要配置 redis_addr 和 redis_passwd
...
auth_key | string | 认证秘钥 | stevegame.cn | stevegame.cn | 用来验证认证 token ，需要和登录服的 auth_key 配置相同
...
* 在测试环境中，建议部署 2 个 gateway 服务
* 在开发环境中，建议部署 1 个 gateway 服务
...
配置项 | 类型 | 作用 | 默认值 | 示例 |备注 
----- | ---- |-------- | ----- | ----- | ------
peipai_addr | string | 内网测试配牌地址 | 127.0.0.1:36102 | 127.0.0.1:36102 | 测试环境需要支持外部访问，生产环境不能支持外部访问
...
* 提供登录认证服务，为用户分配网关，生成用以网关认证的 token
* 创建游戏用户初始数据

### 通用配置项

* 不需要配置 rpc_addr， rpc_port 和 rpc_server_name
...
auth_key | string | 认证秘钥 | stevegame.cn | stevegame.cn | 用来验证认证 token ，需要和网关服的 auth_key 配置相同
...
* 在测试环境和开发环境中，建议部署 1 个 login 服务

#### 负载均衡

* 在生产环境中，应该额外部署第三方代理服务（如 nginx ）用以负载均衡
* 具体流程待完善
