...
func bianzhang(t *testing.T) *utils.DeskData {
	...
	params.Cards = [][]uint32{
		...
		{42, 42, 44, 44, 45, 45, 46, 46, 11, 12, 46, 19, 19},
...
//TestFan_BianZhang_Zimo_ERM 边张立即结算自摸测试
// 庄摸牌46,自摸
//期望赢分：18 = [2（箭刻） + 1（边张） + 6（混一色）  +4（无花）+1（自摸）+4（全带幺）]* 1
func TestFan_BianZhang_Zimo_ERM(t *testing.T) {
	deskData := bianzhang(t)
	//开局 0 自询
