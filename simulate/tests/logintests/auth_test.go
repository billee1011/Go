...
func Test_AuthAgain(t *testing.T) {
	...
	client1 := connect.NewTestClient(config.GetLoginServerAddr(), config.GetClientVersion())
	response1, _ := utils.RequestAuth(client1, accountID, accountName, time.Second*5)

	client2 := connect.NewTestClient(config.GetLoginServerAddr(), config.GetClientVersion())
