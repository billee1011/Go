...
func TestXingPaiOptionManager_loadOption(t *testing.T) {
	...
	assert.Equal(t, opt1.PlayerNum, 4)
	...
	assert.Equal(t, opt2.PlayerNum, 4)

	opt4 := xom.GetXingPaiOption(4)
	assert.Equal(t, opt4.Hnz.Need, false)
	assert.Equal(t, opt4.Hnz.Num, 0)
	assert.Equal(t, opt4.ID, 4)
	assert.Equal(t, opt4.NeedAddflower, true)
	assert.Equal(t, opt4.NeedChi, true)
	assert.Equal(t, opt4.NeedDingque, false)
	assert.Equal(t, opt4.PlayerStates, []XingpaiState{Hu, Giveup})
	assert.Equal(t, len(opt4.WallCards), 72)
	assert.Equal(t, opt4.HuGameOver, false)
	assert.Equal(t, opt4.PlayerNum, 2)
}
